...
function getWelcomeMessage(phaseId: PhaseId): string {
  const welcomes: Record<PhaseId, string> = {
    ...
    investigacion: "Fase 3 — Investigación. Voy a traducir el Programa de Investigación Profunda (PIP) heredado de F2 en un tablero de tareas concretas. Para cada pregunta te propondré el canal más adecuado: activar una app del ecosistema, pedirte que cargues un documento o vincular alguna herramienta que ya usaste por tu cuenta.\n\nAbre \"**Ver tablero**\" cuando quieras revisar las tareas propuestas y aprobar cada asignación. Aquí, en el chat, te avisaré conforme lleguen resultados nuevos.\n\nSi durante la investigación identificas que falta agregar, editar o eliminar alguna pregunta del PIP, regresa a F2-Exploración y usa \"Editar análisis\". **Al volver a F3 y abrir el tablero**, verás un aviso si el PIP cambió desde la última vez, con la opción de sincronizar el tablero automáticamente. No necesitas incorporar los cambios a mano.",
