...
function SectionHeader({ title, scope, scope2 }: { title: string; scope?: string; scope2?: string }) {
  return (
    <div className="mb-3 text-center">
      ...
      {scope && <p className="text-xs text-black-eske-20 dark:text-[#9AAEBE] mt-0.5">{scope}</p>}
      {scope2 && <p className="text-xs text-black-eske-20 dark:text-[#9AAEBE] mt-0.5">{scope2}</p>}
    </div>
  );
}
