import { runFontanaBanxicoDiagnostics } from "@/lib/dev/fontanaBanxicoSandbox";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Sandbox Banxico SIE — Fontana",
...
const ERROR_LABELS: Record<string, string> = {
  token_missing: "Token no configurado (BANXICO_TOKEN ausente)",
  token_invalid: "Token inválido o sin permiso (401/403)",
  series_not_found: "Serie no encontrada en el SIE",
  network_error: "Error de red / HTTP inesperado",
  timeout: "Tiempo de espera agotado (10s)",
  empty_response: "Metadatos existen pero sin observaciones",
  malformed_response: "Formato de respuesta inesperado",
};

export default async function FontanaBanxicoSandboxPage() {
  const results = await runFontanaBanxicoDiagnostics();
  ...
  return (
    <div className="min-h-screen bg-white-eske dark:bg-[#0D2035] p-6">
      <h1 className="text-2xl font-bold text-black-eske dark:text-white mb-1">
        Sandbox Banxico SIE — Fontana (F1-17, remesas)
      </h1>
      <p className="text-sm text-black-eske-60 dark:text-white/50 mb-1 max-w-2xl">
        Diagnóstico de conexión real al Sistema de Información Económica de
        Banxico. No es una página de producto — solo prueba de viabilidad
        técnica de las series de remesas por entidad federativa (cuadro CA79).
      </p>
      <p className="text-xs font-mono text-black-eske-60 dark:text-white/40 mb-5">
        API: SIE REST v1 · series SE29670-SE29702 (CA79) · ejecutado: {ranAt}
      </p>
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs font-mono text-black-eske-60 dark:text-white/50">
                {r.case.serieId} · {r.case.entidad} · {Math.round(r.responseTimeMs)}ms
              </span>
            </div>
            ...
            {r.ok ? (
              <p className="mt-1 text-sm text-black-eske dark:text-white/80">
                ...
                {r.unit ? ` ${r.unit}` : ""} — periodo {r.date}
                {r.periodicidad ? ` (${r.periodicidad})` : ""}
              </p>
            ) : (
