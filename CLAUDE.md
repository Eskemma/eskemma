...
| **Paso 4** — Moddulo (`project.territorio` estructurado en el chat), portabilidad por id, multiselección | 🟡 **En curso, 3 entregas separadas:** **4a ✅ construido 26-09-26** (bloque de territorio compartido + chat + advisor de F2); **4b ✅ construido 26-09-26** (ruta `GET /api/territorio/origen` + wizards por id, sin los 4 query params planos); 4c ⏳ (multiselección de municipios con `clave` calculada en `/api/geo/options` + arreglo del Enter-primero de `PartidosMultiSelect`) |
