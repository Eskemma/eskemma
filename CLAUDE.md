...
| **Paso 4** — Moddulo (`project.territorio` estructurado en el chat), portabilidad por id, multiselección | ✅ **3 entregas separadas (4a, 4b, 4c):** **4a ✅ construido 26-09-26** (bloque de territorio compartido + chat + advisor de F2); **4b ✅ construido 26-09-26** (ruta `GET /api/territorio/origen` + wizards por id, sin los 4 query params planos); **4c ✅ construido 26-09-26** (multiselección de municipios con `clave` calculada en `/api/geo/options` + arreglo del Enter-primero de `PartidosMultiSelect`). **Paso 4 completo** salvo verificación en navegador de 4c |
...
*Paso 2a (construido 26-09-24, decisiones de Raúl).* `MunicipioSeleccionado` gana `clave?: string`
(`14:SAN PEDRO TLAQUEPAQUE`; se llama `clave` y NO `cve` a propósito: es por nombre, no el cve INE). Módulo
puro `lib/geo/municipioSeleccionado.ts` (alta, dedup por clave, reemplazo, `municipiosDeTerritorio`,
`proponerRelleno`) probado con las formas reales guardadas (21 tests). `TerritorySelector`: `handleAgregarMunicipio`
usa `POST /api/geo/candidatos` (único → se agrega con clave; ambiguo → picker que **guarda la clave elegida**,
antes se descartaba; demasiados → pide más nombre; sin reconocer o error de red → se agrega sin clave con aviso,
como antes). **Política de `nombre`:** un exacto conserva lo tecleado (acentos intactos); un alias, un nombre
incompleto o una opción del picker guardan el nombre OFICIAL del catálogo (sin acentos: el catálogo no los trae) y
avisan «Pachuca se interpretó como Pachuca de Soto»; rellenar la clave de una entrada YA guardada nunca la
renombra. **Relleno perezoso:** al abrir un territorio guardado sin claves, cada municipio se resuelve contra el
núcleo; las resoluciones únicas (exactas, alias, parciales) completan la clave, las ambiguas se piden en el
picker («ya guardado… elige cuál para fijarlo») y las de fuera de México se dejan sin clave. **Script puntual**
`scripts/migrar-municipios-clave.ts`: DRY-RUN por defecto; solo escribe con `--apply` y los casos aprobados por
id (`--aprobar=C01,…,exactos`, `--resolver=<caso>=<clave>` para ambiguos), `--simular` imprime el payload sin
escribir, respaldo previo, relee cada documento y aborta si cambió, idempotente, solo toca
`territorio.municipiosPorEstado` (no `municipiosSeleccionados` ni el escalar `municipio`). Dry-run real
(26-09-24): 7 documentos, 28 entradas: 22 únicas exactas, 2 alias (Tlaquepaque), 3 parciales (Tlajomulco ×2,
Acatlán), 1 ambigua (Ixtlahuacán); el resto de las 37 referencias municipales guardadas son 2 de Colombia
(Nueva Granada, Magdalena: sin catálogo, se quedan como texto libre) y 8 cabeceras de distrito (texto, fuera de
alcance). **Ixtlahuacán (`fontana_sesiones/vO9JFif6W3UQc7DlyqPq`), verificado antes de aplicar:** el catálogo INE
tiene ambos municipios (de los Membrillos y del Río); los catálogos NACIONALES del repositorio (Metrópolis
2020 SEDATU/CONAPO/INEGI, descargado en vivo: 7 municipios; SUN: 8) **no** incluyen ni Ixtlahuacán ni Acatlán, pero
la delimitación ESTATAL de Jalisco (decretos del Congreso 23021 de 2009: 8 municipios con Ixtlahuacán de los
Membrillos, y 25400/LX/15: 10, con Acatlán de Juárez y Zapotlanejo; fuentes secundarias: Wikipedia «Zona
metropolitana de Guadalajara» e Hispanopedia «Área Metropolitana de Guadalajara»; el sitio de IMEPLAN no es legible
sin JavaScript) sí incluye Ixtlahuacán **de los Membrillos** y en ninguna aparece «del Río». Los 10 municipios de la
sesión son exactamente los 10 de la delimitación estatal. *Límite:* no se leyó el texto primario de los decretos.
Con esto el caso se resuelve a `14:IXTLAHUACAN DE LOS MEMBRILLOS` (la entrada pasa a llamarse «Ixtlahuacan de los
Membrillos»). **Pendiente aparte, decisión de Raúl (no propuesto ni tocado):** el proyecto ZMG
`moddulo_projects/O2RBnCPiyGJ6u6kyk1rS` lista 8 municipios y la sesión vinculada 10; qué territorio cubre
...
/
├── app/
│   ├── api/                          # 143 API route handlers (recontado 26-09-26: 143 (+`territorio/origen`, Paso 4b); 26-09-24 +`geo/candidatos`, −`geo/resolver-municipio`)
...
| Denominador de F1-3 (% Población indígena): `POBTOT` vs. «3 años y más» — PENDIENTE DE CONFIRMAR contra INEGI | **Discrepancia entre definición y cálculo.** `data/fontana/INDICATOR_REGISTRY.json` (F1-3, `definicion`) dice «Proporción de la población de **3 años y más** que habla alguna lengua indígena», pero el cálculo divide `P3YM_HLI` (hablantes de lengua indígena de 3 años y más) entre **`POBTOT`** (población total): `lib/fontana/ingesta/eceg.ts:67` (`"F1-3": { key: "P3YM_HLI", tipo: "porcentaje", denominadorKey: "POBTOT" }`). Los 5 niveles (nacional/estatal/municipal/distrital federal y local) usan la misma configuración, así que el % queda subestimado en todos (el denominador incluye a los menores de 3 años). El mismo denominador aparece en Sefix (`lib/sefix/ecegConstants.ts:157`, `ECEG_DENOMINATORS`: `P3YM_HLI: "POBTOT"`, junto con otros indicadores demográficos «→ POBTOT»), por lo que puede ser una convención deliberada del catálogo ECEG y no un error puntual. El agente además describe el dato como «de 3 años y más» en sus respuestas. Valores de referencia (bodega `sefix/eceg_2020`, Yucatán): estatal 525,092 / 2,320,898 = 22.62 %; D.F. 3102 87,309 / 386,061 = 22.62 % (numeradores y denominadores distintos: casualidad de redondeo, verificada 26-09-26). **Por hacer (tarea breve, no investigada ni corregida):** (1) confirmar contra la definición oficial de INEGI (Censo 2020, ITER/cuestionario ampliado) cuál es el denominador estándar del indicador de población hablante de lengua indígena; (2) ver si la bodega trae `P_3YMAS` como columna (el 26-09-26 solo se leyó `POBTOT` y `P3YM_HLI`) — si no, el pipeline `scripts/eceg-data-pipeline.ts` tendría que incorporarla y re-subir la bodega; (3) según el resultado, o cambiar el denominador (Fontana y, coordinado, Sefix) o cambiar la `definicion` del registry y el texto que el agente da al usuario; (4) revisar si otros indicadores de `ECEG_DENOMINATORS` con base etaria propia (p. ej. `P3HLINHE`, `P_60YMAS`, `P_18YMAS`) tienen el mismo desajuste entre definición y denominador. | 26-09-26, verificación del guard de reuso (F1-3, D.F. 3102 Yucatán) |
...
| 26-09-26 | Fontana — chat: cobertura distrital graduada + guard de reuso de respuestas viejas (incidente sesión `m4qUAX…`, D.F. 3102 Yucatán) | **Falla:** el chat dijo «No existe dato directo a nivel distrital» para F1-3 mientras el panel mostraba 22.62 % (Estimación agregada / Cobertura incompleta). **Causa (2 partes):** (1) el historial que recibe el modelo es solo texto: repitió una ausencia dicha en un turno viejo SIN llamar herramienta; (2) `nivelesComparados` descartaba `coberturaPct`/`tipoDistritoPropio`, así que el chat no podía decir cuánta población cubría el dato. El dato es el MISMO que el de Sefix (Censo 2020 por sección agregado al distrito con la cartografía vigente); «estimación agregada» no es baja confianza. **Misma clase de fallo que la pirámide de Tlaquepaque (respuesta con datos sin llamada a herramienta): patrón recurrente — si aparece un tercer caso, atenderlo como problema de diseño del historial (los `tool_result` no viajan entre turnos), no solo con otro guard.** **Corrección:** `lib/fontana/coberturaDistrital.ts` (puro; umbrales de Raúl: ≥ 99 % sin aviso, 90–99 % nota breve, < 90 % advertencia fuerte; mismo criterio en chat y panel — antes el panel avisaba igual a 98.4 % que a 64.1 %); `lib/fontana/agente/nivelesComparados.ts` (extraído de `tools.ts`; expone `coberturaPct`, `tipoDistritoPropio` y `avisoCobertura` en `nivelesComparados` y en la consulta por nivel); `CoberturaAdvertencia` gradúa etiqueta/color («Nota sobre cobertura» azul / «Cobertura parcial» naranja) y texto (porcentaje vs conteo); prompt: secciones «Cobertura del dato distrital» y «Datos de turnos anteriores: no los reutilices» + plantilla de fuente corregida (Censo/ECEG = «cruzando secciones con los límites electorales», no «combinando municipios»); guard de servidor `lib/fontana/agente/guardReuso.ts` + `chat/route.ts` (`correccionReusoHecha`): si el turno cierra sin herramientas y la respuesta trae tabla de valores o afirma ausencia de dato → text_suppress + una re-consulta forzada (omitido con adjuntos). **Verificación:** 603 pruebas (cobertura con los 6 distritos reales de Yucatán 98.7/98.4/72.8/72.8/92.9/64.1; guard con el texto real de la ausencia; `nivelesComparados` con la celda real), `tsc`, `next build`, functions build, `check-geo-cf`; llamada REAL a Claude con historial que contiene la ausencia vieja: reconsulta y responde 22.62 % con «cubre aproximadamente el 98.4 %…» (el prompt bastó). **Guard probado después en la ruta REAL** (`POST /api/fontana/chat` con la primera respuesta del modelo forzada a la ausencia vieja, sin escribir en la sesión): emitió `text_suppress`, inyectó el aviso, el modelo reconsultó (2 llamadas a herramienta) y el usuario vio 22.62 % con su cobertura; sin el prompt nuevo el modelo tampoco reutilizó la respuesta vieja en 4 corridas (la falla natural no se pudo provocar). **Coincidencia 22.62 % estatal = distrital: casualidad genuina** (525,092/2,320,898 vs 87,309/386,061; 0 de 979 registros distritales copian el estatal; solo 5 coinciden al redondear). Hallazgo derivado: denominador de F1-3, ver Deuda Técnica. F2-1 real del mismo distrito da 55.02 % con cobertura 63.8 % → advertencia fuerte. **Observación NO tratada:** en F1-3 el valor estatal de Yucatán y el distrital de D.F. 3102 son idénticos (22.62); conviene revisar si es coincidencia o el cálculo distrital/estatal comparte origen. Sin reparación de datos (historial append-only; basta volver a preguntar). Fuera de alcance: nota de cobertura en Sefix. **Pendiente de Raúl (navegador):** repetir la pregunta en la sesión `m4qUAX…` y ver el chip graduado en el panel. |
| 26-09-26 | Desambiguación geográfica — Paso 4a: territorio estructurado en el chat y el advisor de Moddulo | **Hueco:** el chat de Moddulo nunca recibía `project.territorio` y el prompt de F2 ordenaba «infiere el país/estado a partir del XPCTO». **Construido:** módulo puro `lib/geo/bloqueTerritorio.ts` (extraído de Fontana SIN cambiar su salida: 36 combinaciones territorio×tipo idénticas byte a byte contra el código anterior; expone `lineasBaseTerritorio`, `unidadesPlurales(t, tope?)`, `bloqueTerritorioModdulo`); `getPhaseSystemPrompt` gana el 4.º parámetro `territorio` (sección ADICIONAL tras el XPCTO y antes de los datos capturados; "" si no hay territorio → proyectos legados intactos); `chat/[phaseId]/route.ts` pasa `project.territorio`; `f2/advisor/route.ts` incluye el mismo bloque. Bloque: nivel, territorio canónico (`D.F. 3102 PROGRESO (Yucatán)`), estado/municipio, país (o «no declarado, no asumas uno»), lista plural con tope de 20 + «y N más» y 3 reglas («este distrito» = el del bloque; confirmar si nombra otro lugar; el chat no edita el territorio). **Decisiones de Raúl:** chat SIN herramientas (dar tools al chat es otra ronda); sin claves en el prompt. La frase de F2 se conserva y se le antepone «si existe el bloque TERRITORIO, úsalo; infiere solo si no existe» (único texto existente modificado). **Grounding:** el bloque va SOLO al system prompt, fuera de `mergedXpctoContext` (que alimenta `textosContexto`); prueba: un código de distrito extraído como cifra del usuario sigue siendo descartado. **Tamaño medido:** prompts base 6.3–11.9 mil caracteres (≈1.8–3.4 mil tokens); bloque peor caso (20 unidades) ≈ 720 caracteres (< 12 % del prompt base en las 9 fases, con prueba). **Verificación:** 641 pruebas (caracterización de las 9 fases contra `lib/ai/phases/__fixtures__/prompts-antes-4a.json`: SIN territorio idéntico byte a byte —salvo la frase de F2—, CON territorio superset; aserción del 4.º argumento probada en negativo; 11 casos del módulo: nacional, estatal, D.F./D.L. canónico, ZMG con y sin tope, legado, país no declarado, extranjero sin claves), `tsc`, `next build`, functions build, `check-geo-cf`; llamadas REALES a Claude con proyectos reales: Iztapalapa (D.L. 27) en F1/F2/F3 — cada fase sigue explicando su etapa y ubica «este distrito» en el distrito real — y ZMG (F2) — lista los 8 municipios guardados sin inventar 10. **Pendiente de Raúl (navegador):** el chat de un proyecto de distrito preguntando «¿qué hago en esta etapa?» y «¿en qué territorio estoy?». |
| 26-09-26 | Desambiguación geográfica — Paso 4b: portabilidad del territorio por id entre Moddulo, PESTEL y Fontana | **Antes:** Moddulo→PESTEL reconstruía el territorio desde 4 query params de texto (`nivel/estado/municipio/pais`) y perdía `cve_distrito` y las 4 listas plurales; PESTEL→Moddulo no pasaba territorio (el usuario lo reescribía). **Construido:** ruta única `GET /api/territorio/origen?app=moddulo|pestel|fontana&id=` (`lib/territorio/origenTerritorio.ts`): sesión obligatoria (401), `app` fuera de la lista o sin id → 400, y **el MISMO 404** para origen inexistente, ajeno, mal formado o sin territorio (no es un oráculo de existencia; NO se copiaron los 403 de `import-pestel`/`export-to-moddulo` ni el 200 `{found:false}` de `find-linked-pestel`). Devuelve solo `{territorio, nombre, tipo, color}`. Guards: `getProjectParaPrellenado` (Moddulo, mismo criterio que `getProject` —colaborador— pero de SOLO LECTURA: sin `lastAccessedAt` ni backfills), helper NUEVO `getPestelProjectPropio` (`lib/centinela/pestel/projectPropio.ts`, `userId === uid`; ahora lo usan también `createProject` y `GET pestel/project/[projectId]`) y `cargarSesionConTerritorioActual` (Fontana, con resincronización desde el proyecto vinculado). Wizards: `pestel/nuevo` prellena con `moddulo_project_id` (efecto; no pisa lo ya elegido), `moddulo/proyecto/nuevo` prellena desde PESTEL (`pestelProjectId`, antes nada) y la rama Fontana pasa a la misma ruta (antes `GET /api/fontana/sesion?sesion_id`, que no resincronizaba). Retirados los 4 params planos de `exploracion/page.tsx` (un solo camino). **Decisión de Raúl:** criterio de propiedad = el de `getProject` (colaborador); la política de roles de Moddulo sigue pendiente aparte. **Evidencia real (Firestore, 13 proyectos Moddulo con territorio):** 5 tienen plural o `cve_distrito`; con el mecanismo anterior los 5 pierden datos (5/5), con la ruta por id los 5 llegan íntegros (5/5, comparación profunda); PESTEL 5/5 y Fontana 14/14 se resuelven por id. **Guard probado en negativo en las 3 apps** (quitar cada guard hace fallar la prueba de «ajeno = inexistente»; el de PESTEL además rompe `createProject.test`). 12 pruebas de la ruta + 653 en total, `tsc`, `next build`, functions build, `check-geo-cf`. **Hueco de datos investigado (pregunta de Raúl):** el proyecto de Iztapalapa (`nZvpYu…`) SÍ tiene `distritosSeleccionados` (1) y `cve_distrito` 027 — **mi nota de 4a decía lo contrario por inferirlo del nombre sin leer el documento; era incorrecta.** El hueco real son 7 documentos LEGADOS de nivel distrito sin `distritosSeleccionados`: 4 con «Distrito Electoral Federal V, con cabecera en Puerto Vallarta» (2 Moddulo `FK83…`/`fvpu…`, 1 PESTEL `okcc…`, 2 sesiones Fontana vinculadas) creados 2026-07-13/14, y la sesión Fontana `1qEjT…` («Distrito Electoral Local 27 en la CDMX», sin `cve_distrito`, vinculada a `nZvpYu…`, cuyo territorio se resincroniza desde el proyecto); ninguno se creó con el selector estructurado (el de Iztapalapa, 2026-08-04, y `m4qUAX…` sí lo pueblan): los flujos actuales lo pueblan bien y el sistema compensa en los legados por texto (`extraerNumeroDistrito`, `buscarDistritoCandidatos`). No se tocó ni se migró: **pendiente de decisión de Raúl** un backfill puntual con el patrón de `migrar-municipios-clave.ts` (dry-run, aprobación por caso). **Pendiente de Raúl (navegador):** «Abrir PESTEL» desde F2 de un proyecto plural (ZMG o Iztapalapa) y ver el territorio íntegro en el paso 2 del wizard; «Iniciar proyecto en Moddulo» desde un proyecto PESTEL con territorio. |
| 26-09-26 | Backfill de `distritosSeleccionados` en 6 documentos legados de nivel distrito (datos, aparte del código de 4b) | `scripts/migrar-distritos-seleccionados.ts` (mismo patrón que `migrar-municipios-clave.ts`: dry-run por defecto, aprobación por caso, `--simular`, respaldo, relee y aborta si el documento cambió, idempotente). **Corrección:** eran **6** documentos, no 7 (mi conteo de 4b estaba mal): 5 de «Distrito Electoral Federal V, con cabecera en Puerto Vallarta» (Moddulo `FK83…`/`fvpu…`, PESTEL `okcc…`, Fontana `p6RJ…`/`yzjw…`) y la sesión Fontana `1qEjT…` («Distrito Electoral Local 27 en la CDMX», sin `cve_distrito`). Resolución con el catálogo vigente: número + estado → código de 4 dígitos, confirmado por la cabecera del texto (5 casos) o por el proyecto Moddulo vinculado (1 caso). Aprobados D01–D06 por Raúl; se simuló el payload y coincidió. **Escrito:** solo `territorio.cve_distrito` (3 dígitos: `"005"`, `"027"`) y `territorio.distritosSeleccionados` (`[{cve, nombre: cabecera, estado}]`); `nombre`, `municipio` (frase legada) y `estado` intactos. **Verificado por relectura:** los 6 documentos; los 3 pares vinculados (`1qEjT↔nZvpYu`, `p6RJ↔FK83`, `yzjw↔fvpu`) pasaron de `approximate` a **`exact`** en `checkTerritoryMatch`; las celdas distritales de Fontana siguen resolviendo (D.F. 1405 = 281,404; D.L. 0927 = 258,386). Respaldo del estado anterior en el scratchpad de la sesión. |
| 26-09-26 | Desambiguación geográfica — Paso 4c: multiselección de municipios con búsqueda + Enter-primero | **`/api/geo/options?tipo=municipios` ahora trae `clave`** (campo aditivo) calculada en el servidor con la MISMA regla que `desambiguarReferencia` (`lib/geo/clavesMunicipioCatalogo.ts`: `${estadoCve}:${claveCanonica}`, `#cve` solo en los 4 homónimos de Oaxaca). **Medido con datos reales:** Oaxaca 570 opciones, ~44 KB (más que los ~30 KB estimados en el plan), 570/570 claves únicas y 4 con `#cve`; Jalisco 125/125; cruce con el núcleo: 570/570 y 125/125 con la misma clave; primera carga en frío ~6.5 s (baja el TopoJSON del estado; después caché de 1 día). `TerritorySelector` (compartido con Fontana y PESTEL): en nivel municipal cada estado tiene un `PartidosMultiSelect` alimentado por `useGeoOptionsMultiEstado({tipo:"municipios"})` (valor = clave, etiqueta con «Dto. NN» en homónimos); selección → `MunicipioSeleccionado {nombre oficial, estado, clave}` con la dedup de siempre (`lib/geo/municipiosMultiselect.ts`). **La alta libre se conserva** («¿No aparece en la lista? Agrégalo escribiendo su nombre») para nombres no encontrados y países sin catálogo (Colombia, sin clave); si el catálogo de un estado no carga se avisa y solo queda la alta libre. Las entradas sin clave siguen como chips aparte; el resto se ve como etiquetas del selector. **Enter-primero** (`PartidosMultiSelect`, compartido con Sefix): antes Enter agregaba SIEMPRE la primera opción (incluso con búsqueda vacía o el desplegable cerrado) y sin coincidencias podía enviar un `<form>`; ahora `lib/ui/multiSelectFiltro.ts` (`decidirEnter`): solo agrega la primera coincidencia con el desplegable abierto y texto escrito, siempre evita el envío del formulario, ignora Enter durante composición IME. Además: búsqueda sin acentos/mayúsculas («tonala» → Tonalá) y tope de 100 opciones dibujadas con aviso «Mostrando N de M». **Verificación:** 671 pruebas (18 nuevas: claves de los 32 estados, homónimos de Oaxaca —seleccionar/quitar uno conserva el otro—, dedup, entradas sin clave/otros estados intactos, Enter con búsqueda vacía/cerrado/sin coincidencias/con coincidencia/IME, filtro de 570), `tsc`, `next build`, functions build, `check-geo-cf`. **Límite:** el cableado del componente no tiene prueba de DOM (no hay entorno de navegador en la suite): la lógica está probada en módulos puros y el resto va en la guía de navegador. |
